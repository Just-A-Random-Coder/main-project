...
function validateTime(){
    ...
    if (checkBox && checkBox.checked) {
        ...
        if (selectedTime > oneYearLater) {
            ...
            if (yearsAhead > 10000){
                showAlert("info", `No, Why. This To Do list DOES NOT APPROVE`)
                return 
            }
            else if (yearsAhead > 1000) {
                showAlert("danger", `Really, ${yearsAhead} years?`)
                return
            }
            else if (yearsAhead > 100){
                showAlert("danger", `You will most likely be dead in ${yearsAhead} years so&nbsp;<i><strong>no</strong></i>`)
                return
            } else if (yearsAhead > 10){
                showAlert("danger", `Do you really think you can go ${yearsAhead} years without restarting your computer?`)
                return
            } else if (yearsAhead >= 1){
                showAlert("warning", "No, Just No, That's to long into the future")
                return
            }
        }
    }
...
document.addEventListener("DOMContentLoaded", randomHolder())
