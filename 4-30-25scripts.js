...
function compTime(){
    ...
    return `${selectedTime.toDateString()} ${selectedTime.toTimeString()}`
}

function validateTime(){
    const sDate = compTime()
    const d = new Date()
}
...
function addItems() {
    ...
    if (!value) {
        ...
        value = `${value} <strong>at</strong> ${readTime()}`
...
function loadItems(){
    ...
    const OldTimeItems = localStorage.getItem(STORAGE_KEY1)
