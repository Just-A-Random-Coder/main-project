...
function timeToUseable(string){
    const inputTime = document.getElementById("meetingTime")
    let inTimeM = inputTime.value()
    let ymde = inTimeM.split("-")
    let year = ymde[0]
    let month = ymde[1]
    let dateetc = ymde[2]
    let de = dateetc.split("T")
    let date = de[0]
    let etc = de[1]
    let etcd = etc.split(":")
    let hours = etcd[0]
