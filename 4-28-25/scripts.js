...
setInterval(checkTime, 45000); // Runs every 45 seconds
...
function updateDebugOutput(message) {
    ...
    if (debugDiv) {
        debugDiv.innerHTML = `Debug Info: ${message}`;
    }
}
...
function validateTime() {
    ...
    if (checkBox && checkBox.checked) {
        if (!dateControl || isNaN(dateControl.valueAsNumber)) {
            ...
            dateControl.value=""
            updateDebugOutput("Invalid time input.");
            return false;
        }
        
        const selectedTime = new Date(dateControl.valueAsNumber + new Date().getTimezoneOffset() * 60000); // Already in local timezone
        const now = new Date(); // Current time in local timezone

        // Convert both to UTC for comparison
        const selectedTimeUTC = selectedTime.getTime();
        const nowUTC = now.getTime();
        updateDebugOutput(`Raw Input: ${dateControl.value}, 
                   dateControl.valueAsNumber: ${dateControl.valueAsNumber}, 
                   Selected Time: ${selectedTime}, 
                   Now: ${now}`);
        if (selectedTimeUTC < nowUTC) {
...
function checkTime() {
    if (!timeItems || timeItems.length === 0) return; // If no time-based items exist, exit function.

    const now = new Date(); // Get the current date and time.
    let overdueTasks = []; // Collect overdue tasks for alerting.

    for (const timeItem of timeItems) {
        const [taskDetails, id] = timeItem.split(",").map(item => item.trim()); // Split and extract task details and ID.
        const timePart = taskDetails.match(/at\s(.+)$/i); // Extract the time part using regex.

        if (timePart && timePart[1]) {
            const taskTime = new Date(timePart[1]); // Convert the extracted time to a Date object.

            if (taskTime < now) {
                overdueTasks.push(taskDetails); // Add overdue task to the list.
            }
        }
    }

    // Alert the user if there are overdue tasks.
    if (overdueTasks.length > 0) {
        showAlert("danger", `The following tasks are overdue:<br>${overdueTasks.join("<br>")}`, true);
    }
}
...
function addItems() {
    ...
    if (!value) {
        ...
        timeValue = `${value}, ${items.length+1}`
        timeItems.push(timeValue)
    }
...
function removeItems(idx) {
    const removedItem = items.splice(idx, 1)[0];
    timeItems = timeItems.filter(item => !item.includes(removedItem));
    renderItems();
    saveItems();
}
...
function loadItems(){
    ...
    const OldTimeItems = localStorage.getItem(storageKey1)
    if (OldTimeItems) timeItems = JSON.parse(OldTimeItems);
...
function saveItems(){
    ...
    const stringTimeItems = JSON.stringify(timeItems)
    localStorage.setItem(storageKey, stringItems);
    localStorage.setItem(storageKey1, stringTimeItems)
}
...
dateControl.addEventListener("keydown", function(event){
    if (event.key == "Enter"){
        addItems()
        randomHolder()
    }
...
function showAlert(className, text, persistant){
...
function randomHolder() {
    ...
    if (currentMonth === 11 && currentDay === 15) {
        console.log("It's the developer's birthday today!")
        input.placeholder = `An example would be: ${specialPlaceholder}`;
    } else {
        ...
        while (true) {
            ...
            input.placeholder = `An example would be: ${placeHolders[randomIdx]}`;
